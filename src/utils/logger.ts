...
const wrapLogger = (logger: Logger, moduleName: string, logLevel?: LogLevel) => {
    ...
    const result = new class implements WrappedLogger {
        ...
        /* eslint-disable @typescript-eslint/no-explicit-any */
        trace(...args: any[]): void {
            const tracePrefix = `${COLORS.magenta}[TRACE]${COLORS.default} ${(new Date()).toISOString()} ${moduleName}`;
            if (isTrace) {
        ...
        /* eslint-disable @typescript-eslint/no-explicit-any */
        debug(...args: any[]): void {
            const debugPrefix = `${COLORS.cyan}[DEBUG]${COLORS.default} ${(new Date()).toISOString()} ${moduleName}`;
            if (isDebug) {
        ...
        /* eslint-disable @typescript-eslint/no-explicit-any */
        info(...args: any[]): void {
            const infoPrefix = `${COLORS.green}[INFO]${COLORS.default} ${(new Date()).toISOString()} ${moduleName}`;
            if (isInfo) {
        ...
        /* eslint-disable @typescript-eslint/no-explicit-any */
        warn(...args: any[]): void {
            const warnPrefix = `${COLORS.yellow}[WARN]${COLORS.default} ${(new Date()).toISOString()} ${moduleName}`;
            if (isWarning) {
        ...
        /* eslint-disable @typescript-eslint/no-explicit-any */
        error(...args: any[]): void {
            const errorPrefix = `${COLORS.red}[ERROR]${COLORS.default} ${(new Date()).toISOString()} ${moduleName}`;
            if (isError) {
