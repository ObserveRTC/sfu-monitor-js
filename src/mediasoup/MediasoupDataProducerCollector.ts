...
const logger = createLogger(`MediasoupDataProducerCollector`);
...
export type MediasoupDataProducerCollectorConfig = {
     ...
     */
    pollStats?: (dataProducerId: string) => boolean;
...
export class MediasoupDataProducerCollector implements Collector {
    ...
    public async collect(): Promise<void> {
        ...
        if (this._config.pollStats === undefined || this._config.pollStats(this._dataProducer.id) === false) {
    ...
    public close(): void {
        ...
        this._statsWriter?.removeSctpChannel(this._dataProducer.id);
        this._closed = true;
